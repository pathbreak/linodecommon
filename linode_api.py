...
api_key = os.getenv('LINODE_API_KEY', None)
if (api_key is None):
    print "Error : LINODE_API_KEY environment var is not defined"
    sys.exit(1)
...
def get_datacenter(datacenter, dcs = None):
    if dcs is None:
        dcs = linode_request('avail.datacenters', None)['DATA']
        
    if str(datacenter).isdigit():
...
def create_disk(linode_id, disk_type, disk_size, label, 
    distribution = None, root_password = None, root_ssh_key_file = None):
    ...
    if distribution:
        ...
        if distribution_id is None:
            return (False, None, None, ['Invalid distribution'])
            
        params['FromDistributionID'] = distribution_id

    if root_password:
        params['rootPass'] = root_password
    ...
    return (True, disk_id, job_id, None)




def create_swap_disk(linode_id, swap_disk_size_mb = None):
    # From https://www.linode.com/api/linode/linode.disk.create
    
    if swap_disk_size_mb is None:
        ram_mb = int(get_node_memory(linode_id))
        swap_disk_size_mb = calc_swap_disk_size(ram_mb)
    else:
        swap_disk_size_mb = int(swap_disk_size_mb)
...
def calc_swap_disk_size(ram_mb):
...
def create_diskimage(linode_id, disk_id, image_label):
    ...
    return (True, image_id, job_id, None)


def create_duplicate_disk(linode_id, disk_id):
    # https://www.linode.com/api/linode/linode.disk.duplicate
    resp=linode_request('linode.disk.duplicate', 
        {
            'LinodeID' : linode_id,
            'DiskID' : disk_id
        }
    ...
    job_id = resp['DATA']['JobID']
    new_disk_id = resp['DATA']['DiskID']
    return (True, new_disk_id, job_id, None)


def delete_disk(linode_id, disk_id):
    
    resp = linode_request('linode.disk.delete', 
        {
            'LinodeID' : linode_id,
            'DiskID' : disk_id
        }
...
def create_config(linode_id, kernel, disks, config_label, do_validations=True):
    ...
    # The disks parameter to Linode API should be a comma separated list of disk IDs.
    disk_list = ','.join(map(str, disks))
...
if __name__ == '__main__':
    ...
    if (cmd == 'datacenters'):
        ...
        success, linode_id, errors = create_node(plan, datacenter, do_validations)
        if not success:
        ...
        success, linode_id, errors = update_node(linode_id, label, display_group)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, linode_id, errors = delete_node(linode_id, skip_checks)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, disk_id, job_id, errors = create_disk_from_distribution(linode_id, distribution, 
            disk_size, root_password, root_ssh_key_file)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, disk_id, job_id, errors = create_swap_disk(linode_id)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
    ...
    elif (cmd == 'delete-disk'):
        # Output: Nothing
        # Returns: 0 on success or 1 on failure. Error details on stderr
        linode_id = int(sys.argv[2])
        disk_id = int(sys.argv[3])
        success, job_id, errors = delete_disk(linode_id, disk_id)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, config_id, errors = create_config(linode_id, kernel, disks, config_label, do_validations)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, job_id, errors = boot_node(linode_id, config_id)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, job_id, errors = shutdown_node(int(sys.argv[2]))
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, new_linode_id, errors = clone_node(linode_id)
        ...
        success, image_id, job_id, errors = create_diskimage(linode_id, disk_id, image_label)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
        ...
        success, disk_id, job_id, errors = create_disk_from_image(linode_id, image_id, label, disk_size, 
        ...
        success, _, errors = delete_image(image_id)
        if not success:
            print >>sys.stderr, errors
            sys.exit(1)
